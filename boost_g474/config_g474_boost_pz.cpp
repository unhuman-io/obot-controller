...
#include "config_g474_boost_pz_types.h"
...
struct InitCode {
  InitCode() {
    ...
    RCC->APB1ENR1 =
        RCC_APB1ENR1_SPI3EN | RCC_APB1ENR1_TIM2EN | RCC_APB1ENR1_TIM5EN;
    ...
    SPI1->CR1 =
        SPI_CR1_MSTR | (5 << SPI_CR1_BR_Pos) | SPI_CR1_SPE;  // baud = clock/64
    ...
    // SPI1->CR1 = SPI_CR1_CPHA | SPI_CR1_MSTR | (4 << SPI_CR1_BR_Pos) |
    // SPI_CR1_SSI | SPI_CR1_SSM | SPI_CR1_SPE;    // baud = clock/32 SPI1->CR2
    // = (7 << SPI_CR2_DS_Pos) | SPI_CR2_FRXTH;    // 8 bit

    // // SPI3 MA732
    // SPI3->CR1 = SPI_CR1_MSTR | (3 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
    // SPI_CR1_SSM | SPI_CR1_SPE;    // baud = clock/16 SPI3->CR2 = (15 <<
    // SPI_CR2_DS_Pos);    // 16 bit

    // SPI3 PZ
    DMAMUX1_Channel0->CCR = DMA_REQUEST_SPI3_TX;
    DMAMUX1_Channel1->CCR = DMA_REQUEST_SPI3_RX;
    SPI3->CR1 = SPI_CR1_MSTR | (3 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
                SPI_CR1_SSM | SPI_CR1_SPE;  // baud = clock/16 spi mode 0
...
static struct {
  ...
  HRPWM motor_pwm = {pwm_frequency, *HRTIM1, 3, 5, 4,
                     false,         200};  // driver has 100ns turn off delay,
                                           // then 50 ns deadband. Must avoid
                                           // both to reduce jitter to 20 ns
  FastLoop fast_loop = {(int32_t)pwm_frequency,
                        ...
                        &ADC3->JDR1,
                        &ADC1->DR};
  LED led = {const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM4->CCR1)),
             const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM4->CCR2)),
             const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM4->CCR3))};
  ...
  JointPositionController joint_position_controller = {(float)1.0 /
                                                       main_loop_frequency};
...
uint32_t get_drv_status() {
  ...
  // SPI1 ADS1235
  SPI1->CR1 = SPI_CR1_CPHA | SPI_CR1_MSTR | (4 << SPI_CR1_BR_Pos) |
              SPI_CR1_SSI | SPI_CR1_SSM | SPI_CR1_SPE;  // baud = clock/32
  SPI1->CR2 = (7 << SPI_CR2_DS_Pos) | SPI_CR2_FRXTH | SPI_CR2_RXDMAEN |
...
// float get_vd() {
//     FOC::Vdq0 vdq0;
//     FOC::calculate_vdq0(&vdq0,
//     config_items.main_loop.status_.fast_loop.foc_status.measured.cos,
...
void config_init() {
  System::api.add_api_variable(
      "spi",
      new APICallback([]() { return config_items.spi_debug.read(); },
...
void system_init() {
  ...
  // std::function<void(uint32_t)> setbct = std::bind(&MA732Encoder::set_bct,
  // &config_items.motor_encoder, std::placeholders::_1);
  // std::function<uint32_t(void)> getbct = std::bind(&MA732Encoder::get_bct,
  // &config_items.motor_encoder); System::api.add_api_variable("mbct", new
  // APICallbackUint32(getbct, setbct));

  // std::function<void(uint32_t)> set_et = std::bind(&MA732Encoder::set_et,
  // &config_items.motor_encoder, std::placeholders::_1);
  // std::function<uint32_t(void)> get_et = std::bind(&MA732Encoder::get_et,
  // &config_items.motor_encoder); System::api.add_api_variable("met", new
  // APICallbackUint32(get_et, set_et));

  // std::function<void(uint32_t)> set_mgt = std::bind(&MA732Encoder::set_mgt,
  // &config_items.motor_encoder, std::placeholders::_1);
  // std::function<uint32_t(void)> get_mgt =
  // std::bind(&MA732Encoder::get_magnetic_field_strength,
  // &config_items.motor_encoder); System::api.add_api_variable("mmgt", new
  // APICallbackUint32(get_mgt, set_mgt));

  System::api.add_api_variable("qepi", new APIUint32((uint32_t *)&TIM5->CCR3));
  System::api.add_api_variable(
      "drv_err", new APICallbackUint32(get_drv_status, drv_reset));
  config_items.main_loop.reserved1_ = reinterpret_cast<uint32_t *>(
      &config_items.main_loop.status_.fast_loop.foc_status.command.v_d);
  config_items.main_loop.reserved2_ = reinterpret_cast<uint32_t *>(
      &config_items.main_loop.status_.fast_loop.foc_status.command.v_q);
}
