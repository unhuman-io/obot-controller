...
struct InitCode {
  InitCode() {
    ...
    SPI3->CR1 = SPI_CR1_CPHA | SPI_CR1_MSTR | (4 << SPI_CR1_BR_Pos) |
...
static struct {
  ...
  SPIDMA spi_dma = {
      *SPI3, torque_sensor_cs, *DMA1_Channel1, *DMA1_Channel2, 1000, 1000};
...
void system_init() {
  ...
  // }
  System::log("torque_sensor_init: " +
              std::to_string(config_items.torque_sensor.init()));
  ...
  System::api.add_api_variable("drv_err",
                               new APICallback(get_drv_status, drv_reset));
