...
void usb_interrupt() { usb.interrupt(); }

extern "C" void SystemClock_Config();

struct InitCode {
  InitCode() { SystemClock_Config(); }
};
...
static struct {
  ...
  PWM_EN motor_pwm = {pwm_frequency,
                      ...
                      *TIM8,
                      enable};
  FastLoop fast_loop = {
      pwm_frequency, motor_pwm, motor_encoder, param->fast_loop_param,
      &I_A_DR,       &I_B_DR,   &I_C_DR,       &V_BUS_DR};
  LED led = {const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM3->CCR1)),
             const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM3->CCR2)),
             const_cast<uint16_t *>(
                 reinterpret_cast<volatile uint16_t *>(&TIM3->CCR4)),
