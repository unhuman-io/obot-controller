...
#include "../../motorlib/internal_communication.h"
#include "../../motorlib/system_controller.h"

using PWM = HRPWM;
using Communication = InternalCommunication;
using SystemCommunication = USBCommunication;
...
namespace config {
    ...
    SystemController system_controller(internal_communication);
    ...
    MainLoop main_loop = {fast_loop, position_controller, torque_controller, impedance_controller, velocity_controller, internal_communication, led, output_encoder, torque_sensor, param->main_loop_param};
};

USBCommunication System::communication_ = {config::usb};
void usb_interrupt() {
...
void system_init() {
    ...
    TIM3->CR1 = TIM_CR1_CEN; // system control loop
    config::usb.connect();
