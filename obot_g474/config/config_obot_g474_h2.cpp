...
struct InitCode {
  InitCode() {
    ...
    SPI1->CR1 = SPI_CR1_MSTR | (3 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
...
// TODO: Fix include order: actuator.h must come near end.
#include "../../motorlib/actuator.h"
#include "../../motorlib/system.h"
// TODO: Fix include order: system.h must come last.
...
MainLoop main_loop(fast_loop, position_controller, torque_controller,
                   impedance_controller, velocity_controller, state_controller,
                   joint_position_controller, System::communication_, led,
                   output_encoder, torque_sensor, driver,
...
void system_init() {
  ...
  config::usb.connect();
  HRTIM1->sMasterRegs.MCR = HRTIM_MCR_TDCEN + HRTIM_MCR_TACEN +
...
void config_init() {
  ...
  System::api.add_api_variable(
      "mfilt", new APICallbackUint32(
                   []() { return config::motor_encoder.get_filt(); },
