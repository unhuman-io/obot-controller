...
struct InitCode {
  InitCode() {
    ...
    SPI1->CR1 = SPI_CR1_MSTR | (5 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
                SPI_CR1_SSM | SPI_CR1_CPOL;  // baud = clock/64
    SPI1->CR1 = SPI_CR1_MSTR | (6 << SPI_CR1_BR_Pos) | SPI_CR1_SSI |
                SPI_CR1_SSM;  // baud = clock/128 qia128
...
SPIDMA spi1_dma2(*SPI1, output_encoder_cs, *DMA1_Channel3,
                 *DMA1_Channel4);  // temporary switch
...
void config_init() {
  ...
  System::api.add_api_variable("mtemp", new const APICallbackFloat([]() {
