...
inline void set_adc_seq(ADC_TypeDef* adc_base, uint8_t ch, uint8_t seq_n) {
  const uint32_t pos_table[] = {ADC_JSQR_JSQ1_Pos, ADC_JSQR_JSQ2_Pos,
                                ADC_JSQR_JSQ3_Pos, ADC_JSQR_JSQ4_Pos};
  const uint32_t mask_table[] = {ADC_JSQR_JSQ1_Msk, ADC_JSQR_JSQ2_Msk,
                                 ADC_JSQR_JSQ3_Msk, ADC_JSQR_JSQ4_Msk};
...
void pin_config_obot_g474_h2() {
  ...
  RCC->APB1ENR1 = RCC_APB1ENR1_SPI3EN | RCC_APB1ENR1_TIM2EN |
                  RCC_APB1ENR1_TIM4EN | RCC_APB1ENR1_TIM5EN |
                  RCC_APB1ENR1_USBEN | RCC_APB1ENR1_I2C1EN |
                  RCC_APB1ENR1_RTCAPBEN | RCC_APB1ENR1_PWREN;
  RCC->APB2ENR |= RCC_APB2ENR_SPI1EN | RCC_APB2ENR_TIM1EN |
                  RCC_APB2ENR_HRTIM1EN | RCC_APB2ENR_SYSCFGEN;
  ...
  // Sleep mode enable
  RCC->APB1SMENR1 = RCC_APB1SMENR1_TIM2SMEN | RCC_APB1SMENR1_TIM4SMEN |
                    RCC_APB1SMENR1_TIM5SMEN | RCC_APB1SMENR1_USBSMEN |
  ...
  RCC->AHB2SMENR = RCC_AHB2SMENR_GPIOASMEN | RCC_AHB2SMENR_GPIOBSMEN |
                   RCC_AHB2SMENR_GPIOCSMEN | RCC_AHB2SMENR_GPIODSMEN;
  ...
  ***************************************************/
  ...
  NVIC_SetPriority(RTC_WKUP_IRQn,
  ...
  ***************************************************/
  ...
  // GPIO Configuration
  GPIO_SETL(B, 7, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            2);  // TIM4 CH2 red
  GPIO_SETL(B, 6, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            2);  // TIM4 CH1 green
  GPIO_SETH(B, 9, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            2);  // TIM4 CH4 blue

  // Timer Configuration
  TIM4->CCMR1 = TIM_CCMR1_OC1PE |
                7 << TIM_CCMR1_OC1M_Pos |  // preload and pwm mode 2
  ...
  ***************************************************/
  ...
  static_assert(CPU_FREQUENCY_HZ / config::main_loop_frequency < 65536,
                "Main loop frequency too low");
  ...
  ***************************************************/
  ...
  GPIO_SETH(B, 14, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 chd1
  GPIO_SETH(B, 15, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 chd2
  GPIO_SETH(A, 8, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 cha1
  GPIO_SETH(A, 9, GPIO_MODE::ALT_FUN, GPIO_SPEED::VERY_HIGH,
            13);  // hrtim1 cha2, pwm enable is managed through hrtim
  ...
  /***************************************************
    ...
    Motor Bus Voltage:
    VSNS_BUS - PB0 - ADC3.12 or ADC1.15 - COMP4.INP - OPAMP2.VINP [or
  OPAMP3.VINP]
...
#define ADC1_CH_VBUS 15
  ...
  enum Adc2Channels { Vbus = 16 };

  enum Adc3Channels { Ia = 13 };

  enum Adc4Channels { Ib = 17 };

  enum Adc5Channels { Ic = 5 };
  ...
  ***************************************************/
  ...
  ADC12_COMMON->CCR =
      ADC_CCR_VSENSESEL | ADC_CCR_VREFEN | 3 << ADC_CCR_CKMODE_Pos;
  // Flush Injected Queue, Enable Overrun, Rising Edge Ext Trigger & Regular
  // Sample Trigger Source = trigger 21 -> hrtim_adc_trg1
  ADC1->CFGR = ADC_CFGR_JQDIS | ADC_CFGR_OVRMOD | 1 << ADC_CFGR_EXTEN_Pos |
               21 << ADC_CFGR_EXTSEL_Pos;
  // Injected Oversample Enable, Regular Oversample enable, Oversampling shift =
  // 8-bits, Oversampling Ratio = 256x
  ADC1->CFGR2 = ADC_CFGR2_JOVSE | ADC_CFGR2_ROVSE | (8 << ADC_CFGR2_OVSS_Pos) |
                (7 << ADC_CFGR2_OVSR_Pos);
  ...
  // ADC1->SMPR2 = 2 << ADC_SMPR2_SMP13_Pos | // 12.5 cycles vbus, 294 ns, 200
  // ns min for opamp1
  ADC1->SMPR2 = 6 << ADC_SMPR2_SMP16_Pos |  // 247.5 cycles interal
                                            // temperature, 5.8us, 5us min
                6 << ADC_SMPR2_SMP18_Pos |  // 247.5 cycles vrefint
                                            // (~1.21V), 5.8us, 4us min
                6 << ADC_SMPR2_SMP15_Pos;   // 247.5 cycles vrefint
                                            // (~1.21V), 5.8us, 4us min
  ...
  ***************************************************/
  ...
  // Flush Injected Queue, Enable Overrun, Rising Edge Ext Trigger & Trigger
  // Source = trigger 21 -> hrtim_adc_trg1
  ADC2->CFGR = ADC_CFGR_JQDIS | ADC_CFGR_OVRMOD | 1 << ADC_CFGR_EXTEN_Pos |
               21 << ADC_CFGR_EXTSEL_Pos;
  // Injected Oversample Enable, Regular Oversample enable, Oversampling shift =
  // 8-bits, Oversampling Ratio = 256x
  ADC2->CFGR2 = ADC_CFGR2_JOVSE | ADC_CFGR2_ROVSE | (8 << ADC_CFGR2_OVSS_Pos) |
                (7 << ADC_CFGR2_OVSR_Pos);  // 256x oversample

  // Injected Sample Queue
  ADC2->JSQR = 8 << ADC_JSQR_JSQ1_Pos | 0 << ADC_JSQR_JL_Pos |
  ...
  ***************************************************/
  ...
  ADC345_COMMON->CCR =
      ADC_CCR_VREFEN | 3 << ADC_CCR_CKMODE_Pos;  // hclk/4 (42.5 MHz)
  ...
  ***************************************************/
  ...
  ADC3->SMPR2 = 2 << ADC_SMPR2_SMP13_Pos;  // 12.5 cycles current_sense, 294 ns,
                                           // 200 ns min for opamp3
  /***************************************************
    ADC4
  ***************************************************/
  // Injected Sample Queue
  ADC4->JSQR = 1 << ADC_JSQR_JEXTEN_Pos | 27 << ADC_JSQR_JEXTSEL_Pos |
               17 << ADC_JSQR_JSQ1_Pos;    // trig 27 hrtim adc_trig1 (injected)
  ADC4->SMPR2 = 2 << ADC_SMPR2_SMP17_Pos;  // 12.5 cycles current_sense, 294 ns,
                                           // 200 ns min for opamp6
  /***************************************************
    ADC5
  ***************************************************/

  // Injected Sample Queue
  ADC5->JSQR = 1 << ADC_JSQR_JEXTEN_Pos | 27 << ADC_JSQR_JEXTSEL_Pos |
               13 << ADC_JSQR_JSQ1_Pos;   // trig 27 hrtim adc_trig1 (injected)
  ADC5->SMPR1 = 2 << ADC_SMPR1_SMP5_Pos;  // 12.5 cycles current_sense, 294 ns,
                                          // 200 ns min for opamp4

  NVIC_SetPriority(ADC5_IRQn,
                   NVIC_EncodePriority(NVIC_GetPriorityGrouping(), 0, 0));
  ...
  ***************************************************/
  ...
  // OPAMP1->CSR = 1 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
  // OPAMP_CSR_HIGHSPEEDEN | OPAMP_CSR_OPAMPINTEN | OPAMP_CSR_OPAMPxEN; //
  // internal follower on pa3/vinp1
  ...
  OPAMP4->CSR = 2 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
                OPAMP_CSR_HIGHSPEEDEN | OPAMP_CSR_OPAMPINTEN |
                OPAMP_CSR_OPAMPxEN;  // internal follower on pb11/vinp2
  OPAMP6->CSR = 0 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
                OPAMP_CSR_HIGHSPEEDEN | OPAMP_CSR_OPAMPINTEN |
                OPAMP_CSR_OPAMPxEN;  // internal follower on pb12/vinp0
  OPAMP3->CSR = 1 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
                OPAMP_CSR_HIGHSPEEDEN | OPAMP_CSR_OPAMPINTEN |
                OPAMP_CSR_OPAMPxEN;  // internal follower on pb13/vinp1
  ...
  OPAMP1->CSR = 0 << OPAMP_CSR_VPSEL_Pos | 3 << OPAMP_CSR_VMSEL_Pos |
                OPAMP_CSR_HIGHSPEEDEN |
                OPAMP_CSR_OPAMPxEN;  // external follower on pa1/vinp0 pa2/vout
  ...
  ***************************************************/
  ...
  GPIO_SETH(A, 10, GPIO_MODE::INPUT, GPIO_SPEED::LOW,
            0);  // EXTI trigger for usb
