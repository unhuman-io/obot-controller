...
mdiag  = mot["mdiag"].get()
print(mdiag)
assert(re.match(r"^[\da-f]{16}$", mdiag) or re.match(r"^[\da-f]{8}$", mdiag))
...
check_var_regex("mdiag_str", r"^(err: [\S\s]* warn: [\S\s]*)|([\S\s]*)$")
...
time.sleep(2)
check_var("mcmd_result", "last command: 0xb0, current command: 0xff, result: 0x80")
check_var("mauto_dig", "ok")
time.sleep(.1)
check_var_regex("mcmd_result", r"last command: 0xb1, current command: 0x(00|b1|ff), result: 0x(00|80)")
check_var("mreadj_dig", "ok")
time.sleep(2)
check_var_regex("mcmd_result", r"last command: 0xb2, current command: 0x(00|b2|ff), result: 0x(00|80)")
...
time.sleep(.1)
check_var("mcmd_result", "last command: 0x41, current command: 0x00, result: 0x00")
